...
public class MediaController {
    ...
    public String convertVideo(final String path) {
        ...
        int bitrate = 15600;//921600
        int rotateRender = 0;

        File cacheFile = new File(
                Environment.getExternalStorageDirectory()
                        + File.separator
                        + Apis.VIDEO_COMPRESSOR_APPLICATION_DIR_NAME
                        + Apis.VIDEO_COMPRESSOR_COMPRESSED_VIDEOS_DIR,
        ...
        if (resultWidth != 0 && resultHeight != 0) {
            ...
            try {
                ...
                Log.e("checkkkkkkkkkkkkkkkkkkk", resultWidth + " " + resultHeight);
                movie.setSize(resultWidth + 100, resultHeight + 100);
        ...
        Log.e("path2", inputFile.getAbsolutePath());
        if (!inputFile.exists()) {
            Log.e("path1", "noooo");
        }
        //inputFile.delete();

        Log.e("chash", cacheFile.getAbsolutePath());
        if (!cacheFile.exists()) {
            Log.e("path1", "noooo");
        }
        return cacheFile.getAbsolutePath();
   ...
   /* public String convertVideos(final String path, String outPath) {
        MediaMetadataRetriever retriever = new MediaMetadataRetriever();
        retriever.setDataSource(path);
        String width = retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_VIDEO_HEIGHT);
        String height = retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_VIDEO_WIDTH);
